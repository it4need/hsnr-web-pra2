...
PUT='-XPUT'
DELETE='-X DELETE'
URL='http://localhost:8080/komponente'
URL2='http://localhost:8080/projekt'
#URL_2='http://localhost:8080/Komponentekomponenten'
...
RESPONSE=$(curl -sS "${H1}" "${H2}" "${PUT}" "${URL}/5" -d '{"name":"Komponente A"}')
testError ${RESPONSE}
TEST_SUCCESS_COUNTER=$(($TEST_SUCCESS_COUNTER-$?))
if [[ "$@" == "-v" ]];then echo -e "\nVerbose:" && echo $(echo $RESPONSE) | json;fi

echo -e "\n\n" && echo -e $(blue "----> Testing POST ${URL} - create allowable")
echo -e "... creating project with id 1...";
curl -sS "${H1}" "${H2}" "${POST}" "${URL2}" -d '{"name":"Projekt A"}' > /dev/null
echo -e "... creating component assosiated with project id 1...\n";
RESPONSE=$(curl -sS "${H1}" "${H2}" "${POST}" "${URL}" -d '{"name":"Komponente A", "project_id": 1}')
testSuccess ${RESPONSE}
TEST_SUCCESS_COUNTER=$(($TEST_SUCCESS_COUNTER-$?))
if [[ "$@" == "-v" ]];then echo -e "\nVerbose:" && echo $(echo $RESPONSE) | json;fi
...
RESPONSE=$(curl -sS "${H1}" "${H2}" "${PUT}" "${URL}/1" -d '{"name":"Komponente B"}')
testSuccess ${RESPONSE}
TEST_SUCCESS_COUNTER=$(($TEST_SUCCESS_COUNTER-$?))
if [[ "$@" == "-v" ]];then echo -e "\nVerbose:" && echo $(echo $RESPONSE) | json;fi
